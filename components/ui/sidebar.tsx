...
/** Access the sidebar's open/collapsed state from any descendant. */
function useSidebar() {
...
function SidebarProvider({
  ...
  // Internal state of the sidebar; can be controlled via `open`/`onOpenChange`.
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Persist the state in a cookie so the server can render the correct
      // sidebar state on the next request (no layout flash).
  ...
  // Keyboard shortcut (mod+B) to toggle the sidebar.
  React.useEffect(() => {
...
function Sidebar({
  ...
  return (
    <div
      ...
      data-slot="sidebar"
    >
      {/* Sidebar gap on desktop — reserves layout space for the fixed rail. */}
      <div
...
function SidebarMenuSkeleton({
  ...
  // Vary the width per instance (50–90%) so a list of skeletons looks
  // natural; derived from useId so renders stay pure and deterministic.
